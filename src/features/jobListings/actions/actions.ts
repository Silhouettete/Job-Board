...
export async function createJobListing(
  ...
  if (
    orgId == null ||
    !(await hasOrgUserPermission("job_listings:job_listing_create"))
  ) {
    return {
      error: true,
...
export async function updateJobListing(
  ...
  if (
    orgId == null ||
    !(await hasOrgUserPermission("job_listings:job_listing_update"))
  ) {
    return {
      error: true,
...
export async function toggleJobListingStatus(id: string) {
  ...
  if (
    !(await hasOrgUserPermission("job_listings:job_listing_change_status")) ||
...
export async function deleteJobListing(id: string) {
  ...
  if (!(await hasOrgUserPermission("job_listings:job_listing_delete"))) {
    return error;
  }
...
export async function getAIJobListingSearchResult(
...
> {
  ...
  const matchedListings = await getMatchedJobListings(data.query, allListings, {
    maxNumberOfJobs: 10,
  });
