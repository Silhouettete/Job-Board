...
import { updateUserNotificationSettings as updateUserNotificationSettingsDb } from "@/features/users/db/userNotificationSetting";
...
export async function updateUserNotificationSettings(
  ...
  if (userId == null) {
    return {
      error: true,
      message: "You must be signed in to update the notification setting",
    };
  }
  const { success, data } =
