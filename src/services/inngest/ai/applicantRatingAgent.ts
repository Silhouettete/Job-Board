...
const saveApplicantRatingTool = createTool({
  ...
  description:
    "save the applicant ranking for a specific job listing in the database",
  parameters: z.object({
  ...
  handler: async ({ jobListingId, rating, userId }) => {
    ...
    return "Successfully saved applicant's ranking score ";
  },
});
export const applicantRatingAgent = createAgent({
