...
import { applicantRatingAgent } from "../ai/applicantRatingAgent";

export const rankApplication = inngest.createFunction(
  { id: "rank-applicants", name: "Rank Applicants" },
  ...
  async ({ step, event }) => {
    ...
    await applicantRatingAgent.run(
      JSON.stringify({ coverLetter, jobListingId, userId })
    );
  }
);
